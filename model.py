...
def preprocess():
    ...
    # Build vocabulary
    max_document_length = max([len(x.split(" ")) for x in x_text])  #最大句子的长度
    vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length) #创建字典
    x = np.array(list(vocab_processor.fit_transform(x_text)))  #Learn the vocabulary dictionary 并将文本转换成对应的 token
    tf.logging.info("Shape of X :{}".format(str(x.shape)))

    # Random shuffle data
    np.random.seed(10)
    shuffle_indices = np.random.permutation(np.arange(len(y))) #生成随机序列  0到 len(y)-1
