...
class DataSet(object):
    ...
    def clean_str(self, string):
        ...
        """
        string = re.sub(r"[^A-Za-z0-9(),!?\'\`]", " ", string) #除A-Za-z0-9(),!?'`外的字符
        string = re.sub(r"\'s", " \'s", string)  #匹配 's，在'前加个空格
        string = re.sub(r"\'ve", " \'ve", string)   #'ve 在'前加个空格
        string = re.sub(r"n\'t", " n\'t", string)  #匹配 't，在'前加个空格
        ...
        string = re.sub(r"\)", " \) ", string) # ）前后加空格
        string = re.sub(r"\?", " \? ", string) # ？前后加空格
        string = re.sub(r"\s{2,}", " ", string) #2个以上的空白字符 替换 为" "
